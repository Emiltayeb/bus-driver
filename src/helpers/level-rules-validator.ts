...
const handelLevel3 = (
  userChoice: UserChoiceOptions,
  cardsInGame: [CardType, CardType, CardType]
) => {
  ...
  highestPrev =
    prevFirstCardValue > prevSecondCardValue
      ? prevFirstCardValue
      : prevSecondCardValue;

  lowestPrev =
    highestPrev === prevFirstCardValue
      ? prevSecondCardValue
      : prevFirstCardValue;
  ...
  if (userChoice === 'outside') {
    ...
    return (
      drawnCardValue === prevFirstCardValue ||
      drawnCardValue === prevSecondCardValue
    );
  }
};
